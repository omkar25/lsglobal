        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8 mb-8 sm:mb-12">
          {products.map((product) => (
            <div key={product.key} className="group text-center">
              {/* Image */}
              <div className="relative aspect-square mb-3 sm:mb-6 overflow-hidden rounded-lg">
                <Image
                  src={product.image}
              ...
              {/* Content */}
              <h3 className="text-sm sm:text-xl font-bold text-[#313639] mb-1.5 sm:mb-3 line-clamp-2">
              ...
              <Link
                href={product.href}
                className="inline-block px-3 sm:px-6 py-1.5 sm:py-2 bg-[#441C14] text-white text-xs sm:text-sm font-semibold hover:bg-[#2a110c] transition-colors"
              >
                {t("readMore")}
              </Link>
            </div>
